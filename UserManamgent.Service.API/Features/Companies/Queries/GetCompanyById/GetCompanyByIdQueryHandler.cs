...
namespace UserManamgent.Service.API.Features.Companies.Queries.GetCompanyById
{
    public class GetCompanyByIdQueryHandler : IRequestHandler<GetCompanyByIdQuery, Result<CompanyDto>>
    {
        ...
        public async Task<Result<CompanyDto>> Handle(GetCompanyByIdQuery request, CancellationToken ct)
        {
            ...
            return Result<CompanyDto>.Success(new CompanyDto { Id = c.Id, Name = c.Name, Email = c.Email, ImageName = c.ImageName, Type = c.Type, CountryId = c.CountryId, CountryNameEn = c.Country?.NameEn, CountryNameAr = c.Country?.NameAr, Status = c.Status, AccountManagerId = c.AccountManagerId, IsActive = c.IsActive, IsProvider = c.IsProvider, CreatedAt = c.CreatedAt, UpdatedAt = c.UpdatedAt }, LocalizationKeys.Company.Fetched);
        }
    }
}
