...
namespace UserManamgent.Service.API.Features.Companies.Commands.CreateCompany
{
    public class CreateCompanyCommandHandler : IRequestHandler<CreateCompanyCommand, Result<CompanyDto>>
    {
        ...
        public async Task<Result<CompanyDto>> Handle(CreateCompanyCommand r, CancellationToken ct)
        {
            ...
            var c = CompanyEntity.Create(r.Name.Trim(), r.Type, r.CountryId, r.TierLevel, r.Status, r.AccountManagerId, curId, r.Email, r.ImageName);
            c.IsProvider = true;
            await repo.AddAsync(c, ct); await _uow.SaveChangesAsync(ct);
            return Result<CompanyDto>.Created(new CompanyDto { Id = c.Id, Name = c.Name, Email = c.Email, ImageName = c.ImageName, Type = c.Type, CountryId = c.CountryId, TierLevel = c.TierLevel, Status = c.Status, AccountManagerId = c.AccountManagerId, IsActive = c.IsActive, IsProvider = c.IsProvider, CreatedAt = c.CreatedAt }, LocalizationKeys.Company.Created);
        }
    }
}
