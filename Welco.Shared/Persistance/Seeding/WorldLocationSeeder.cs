...
namespace Welco.Shared.Persistance.Seeding
{
    ...
    {
        // World countries ISO2 + phone code + names (En/Ar). Ar uses En fallback where translation not hand-crafted.
        ...
        // Capitals / major cities sample per country (at least 1 per country)
        ...
        public static async Task SeedAsync(WelcoDbContext db, ILogger? logger = null)
        {
            ...
            // Refresh countries for city seeding
            ...
            // 2. Cities - ensure at least capital per country
            ...
            {
                // add 2 zones per city if missing
                for (int i = 0; i < 2; i++)
                {
